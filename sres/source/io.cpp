...
void parse_ranges_file (char* buffer, input_params& ip, sres_params& sp) {
	...
	for (; buffer[i] != '\0'; i++) {
		...
		if (rate >= ip.num_dims) {
			cout << term->red << i<<"  The number of rates in the given ranges file does not match the given number of dimensions! Please check that the rates file matches the number of dimensions (" << ip.num_dims << ")." << term->reset << endl;
